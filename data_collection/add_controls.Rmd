...
pat <- read_csv("D:\\PCT_EPO_counts\\patent_counts_direct_epo_pct.csv")
...
df$fuel_pr_reg <- values # cbind values to df
...
merge_dfs <- function(state_names) {
        for (i in state_names){
          avg_temp <- bind_rows(i)
              
          write_csv(avg_temp, "D:\\Controls\avg_temp_all.csv")
        }
}
...
for (i in state_names){
            ...
            df3$Date <-substr(df3$Date,1,nchar(df3$Date)-2) # remove last 2 digits of year var
            
            df3$State <- i # add column w state name 
