...
policy <- read_csv("D:\\policy_database\\program_new.csv") %>% # original policy data
  select(!c(administrator, fromSir, additional_technologies))

state_info <- read_csv("D:\\policy_database\\state.csv")  %>% # remove useless vars
  select(!c(is_territory))

authority <- read_csv("D:\\policy_database\\authority.csv") %>%  # add legislative information
...
policy1 <- left_join(policy, type, by = c("program_type_id" = "id")) %>%  # add policy type information
...
policy2$start_year <- policy2 %>% # change format with lubridate
  pull(start_date) %>% 
  year()

policy2 <- policy2 %>%  # remove old variable
  select(!c(start_date))

policy2$program_category <- ifelse(policy2$program_category == 1, "Financial Incentive", "Regulatory Policy") # rename policy categories
...
imputed_dataset <- read_excel(".\\filtered_policies_by_positive_break.xlsx", sheet = 1)  # out of 550 policies, 350 have missing value in year: recovered around 100 policies
...
policy2 <- read_csv(".\\policy_dataset1.csv", show_col_types = FALSE) %>% 
  filter(!(state_name %in% c("American Samoa", "Federated States of Micronesia", "Marshall Islands", "Palau", "Guam", "N. Mariana Islands", "Virgin Islands", "Puerto Rico")))
...
all_policy_types <- df_policy_type %>% # barplot showing policy types frequency
  mutate(name = fct_reorder(Var1, Freq)) %>%
  ggplot( aes( x=name,y=Freq)) +
...
## Policy category
```{r}
df_policy_cat = as.data.frame(table(policy2$program_category)) # barplot showing policy category frequency
...
Use treemap visualization to grasp overall frequency of different policy types
```{r, fig.height=7, fig.width=7}
...
financial <- ggplot(financial_df, aes(area = Freq, fill = Freq, label = Var1)) +
  ...
  geom_treemap_text(colour = "black",
                    place = "centre",
                    size = 15) +  ggtitle("Financial incentive") +
   scale_fill_viridis()
...
regulatory <- ggplot(regulatory_df, aes(area = Freq, fill = Freq, label = Var1)) +
  ...
  geom_treemap_text(colour = "black",
                    place = "centre",
                    size = 15) +  ggtitle("Regulatory policy") +
  scale_fill_viridis()
...
ggsave(".\\treemaps.png", plot = trees, device = "png", width = 12, height = 8)



```

# Policy attribution - selected policies

Manually selected policies by looking ad dataset - this approach is not shown in main text of thesis, as it is not completely agnostic
...
pols <- read_excel(".\\filtered_policies_by_positive_break.xlsx", sheet = 3) %>% 
  ...
  filter(case_when(state_name == "District of Columbia" ~ year > 2009 & year < 2013, 
                   ...
                   state_name == "Tennessee" ~ year > 1999 & year < 2011, 
                   state_name == "Virginia" ~ year > 1996 & year < 2014))
...
# Version with non-selected policies: shown in master thesis
```{r, fig.height=7, fig.width=17}
...
data_rect <- data.frame(state_name = unique(pols2$state_name), # Create data for conf int
                        rect_min = c(2010, 2006, 2006, 2000, 1997, 2007), 
                        rect_max = c(2012, 2008, 2010, 2010, 2013, 2021))
...
final_policy_breaks <- policy_breaks + panel_border() + geom_rect(data = data_rect, aes(xmin = rect_min, xmax = rect_max, ymin = -Inf, ymax = Inf, group = state_name), fill = "grey", alpha = 0.2, inherit.aes = FALSE)
...
# Version with aggregated policy types
```{r, fig.height=7, fig.width=17}

Funds_to_renewable_energy <- c("Public Benefits Fund", "Grant Program")

Tax_incentives <- c("Property Tax Incentive", "Sales Tax Incentive", "Corporate Tax Credit", "Personal Tax Deduction", "Personal Tax Credit")

Consumer_subsidy <- c("Rebate Program", "Loan Program", "PACE Financing", "Performance-Based Incentive")

Regulatory_policies_for_renewable_energy_diffusion <- c("Building Energy Code", "Green Power Purchasing", "Net Metering", "Interconnection", "Mandatory Utility Green Power Option")

Regulatory_policies_renewable_energy_access <- c("Solar/Wind Access Policy", "Solar/Wind Permitting Standards")

Renewable_energy_and_or_energy_efficiency_standards <- c("Renewables Portfolio Standard" , "Energy Standards for Public Buildings", "Energy Efficiency Resource Standard")
...
pols2 <- read_excel(".\\filtered_policies_by_positive_break.xlsx", sheet = 1) %>% 
  ...
  filter(case_when(state_name == "District of Columbia" ~ start_year > 2009 & start_year < 2013, 
                   ...
                   state_name == "Tennessee" ~ start_year > 1999 & start_year < 2011, 
                   state_name == "Virginia" ~ start_year > 1996 & start_year < 2014)) %>% 
  mutate(policy_type = case_when(policy_type %in% Funds_to_renewable_energy  ~ "Funds for Renewable Energy",
                                 policy_type %in% Tax_incentives   ~ "Tax incentives", 
                   policy_type %in% Regulatory_policies_for_renewable_energy_diffusion ~ "Regulatory Policies for RE Diffusion",
                   policy_type %in% Regulatory_policies_renewable_energy_access  ~ "Regulatory Policies for RE Access",
                   policy_type %in% Consumer_subsidy  ~ "Consumer Subsidy",
                   policy_type %in% Renewable_energy_and_or_energy_efficiency_standards  ~ "RE and/or Energy Efficiency Standards",
                   policy_type == "Industry Recruitment/Support"  ~ "Industry Recruitment/Support"))
...
final_policy_breaks_agg <- policy_breaks + panel_border() + geom_rect(data = data_rect, aes(xmin = rect_min, xmax = rect_max, ymin = -Inf, ymax = Inf, group = state_name), fill = "grey", alpha = 0.2, inherit.aes = FALSE)

final_policy_breaks_agg

ggsave(".\\policy_breaks_agg.png", plot = final_policy_breaks_agg, device = "png", width = 17, height = 10)

```
# Another type of aggregation: demand-pull vs tech-push
...
Consumer_subsidy <- c("Rebate Program", "Loan Program", "PACE Financing")
...
DEMAND_PULL <- c(Tax_incentives, Consumer_subsidy, "Performance-Based Incentive", Regulatory_policies_for_renewable_energy_diffusion)

TECHNOLOGY_PUSH <- c(Funds_to_renewable_energy, "Industry Recruitment/Support")

MIXED <- c(Regulatory_policies_renewable_energy_access, Renewable_energy_and_or_energy_efficiency_standards)
...
pols2 <- read_excel(".\\filtered_policies_by_positive_break.xlsx", sheet = 1) %>% 
  ...
  filter(case_when(state_name == "District of Columbia" ~ start_year > 2009 & start_year < 2013, 
                   ...
                   state_name == "Tennessee" ~ start_year > 1999 & start_year < 2011, 
                   state_name == "Virginia" ~ start_year > 1996 & start_year < 2014)) %>% 
  mutate(category = case_when(policy_type %in% DEMAND_PULL  ~ "Demand Pull",
                              policy_type %in% TECHNOLOGY_PUSH   ~ "Technology Push", 
                              policy_type %in% MIXED ~ "Mixed"))
...
p <- pols2 %>% 
  ggplot(aes(x = start_year, fill = category)) +
  geom_bar() + 
  facet_wrap(~state_name
             ) + 
  theme_cowplot() +
...
final_policy_breaks_demand <- policy_breaks + panel_border()

final_policy_breaks_demand

ggsave(".\\policy_breaks_demand_vstech.png", plot = final_policy_breaks_demand, device = "png", width = 17, height = 10)
...
p <- pols2 %>% 
  ggplot(aes(x = start_year, fill = program_category)) +
  geom_bar() + 
  facet_wrap(~state_name) + 
  theme_cowplot() +
  scale_fill_manual(values=as.vector(alphabet(26))) +
...
final_policy_breaks_regvsfin <- policy_breaks + panel_border()

final_policy_breaks_regvsfin

ggsave(".\\final_policy_breaks_regvsfin.png", plot = final_policy_breaks_regvsfin, device = "png", width = 17, height = 10)


```
# Inspect n of policies found in break interval vs total n of policies 
...
Manually inspect whether there policies in these states that could have been a potential cause for patenting increases
```{r}
