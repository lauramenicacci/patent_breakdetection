...
mod <- readRDS(".\\model_spec\\results\\mods_main_updatedpackage.RDS") # load main model output

mod_noIIS <- readRDS(".\\model_spec\\results\\mods_main_updatedpackage_noIIS.RDS") # load main model output w/o 
```

# Plot total output with counterfactuals
```{r, fig.height=7, fig.width=7}

p_iis <- plot_counterfactual(mod$is[[1]]) + theme_clean() + theme(plot.background = element_blank(), # plot counterfactuals
                                                         legend.background = element_blank(), 
                                                         text=element_text(size=10,  family="serif"))

p_noiis <- plot_counterfactual(mod_noIIS$is[[1]]) + theme_clean() + theme(plot.background = element_blank(), # plot counterfactuals
                                                         legend.background = element_blank(), 
                                                         text=element_text(size=10,  family="serif"))

main_counterfactuals <- plot_grid(p_iis, p_noiis, ncol = 1, labels = c("A. Main specification with IIS", "B. Main specification without IIS"), label_fontfamily = "serif", label_fontface = "plain", scale = 0.9) # put both models in a grid 
...
```{r}
identify_indicator_timings <- function(object){ # this function takes the output of the model with all the dummies and returns the years, the country when the step or impulse was identified
...
# this function plots the y, the fitted values and a vertical red line for the fesis, a vertical grey line for the iis 
...
```{r}
plot_counterfactual <- function(x, plus_t = 5, facet.scales = "free", title = NULL, zero_line = FALSE){ # this function adds the counterfactual using the confidence interval data given by the break uncertainty function 
  ...
  df_ident <- break_uncertainty(x) # this function identifies the 99% confidence interval around the break
...
# this function filters for the states where positive break is identified and only plots counterfactual when break is positive
