...
# Distribution of y (patent counts) with removed states


```{r, echo=FALSE, eval=FALSE}

df %>% 
  ggplot(aes(x = patent_count)) + geom_boxplot() + ggtitle("Total distribution of patent counts")


df %>% 
  filter(state_name %in% nonpat_visual) %>% 
  ggplot(aes(x = patent_count)) + 
  geom_boxplot()+
  ggtitle("Total distribution of patent counts without 5 states selected visually")
  

df %>% 
  filter(state_name %in% nonpat_mechanical) %>% 
  ggplot(aes(x = patent_count)) + 
  geom_boxplot()+
  ggtitle("Total distribution of patent counts without 13 states selected mechanically")
...
Potential problem of this approach: give too much importance to outliers!! Maybe take the 75th quantile? 
