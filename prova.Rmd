...
```{r}
plot_counterfactual(mod$is[[1]]) + theme_clean() + theme(plot.background = element_blank(), 
                                                         legend.background = element_blank(), 
                                                         text=element_text(size=10,  family="serif"))
```

```{r}
break_uncertainty(mod_noIIS$is[[3]])
```
```{r}
mod_noIIS$is[[1]]
...
# Modify function from getspanel to plot only countries with breaks & only countries with positive breaks
```{r}
...
plot_counterfactual_laura_noiis <- function(x, plus_t = 5, facet.scales = "free", title = NULL, zero_line = FALSE){
  ...
  effects_filt <- effects %>% filter(id %in% c("Wisconsin", "SouthCarolina", "Kentucky", "DistrictofColumbia", "Tennessee", "Virginia")) %>% mutate(coef = case_when(coef < 0 ~ coef == NA),
...
iis_res <- plot_counterfactual_laura(mod$is[[1]])
