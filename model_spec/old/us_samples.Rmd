...
# create two samples based on median on gdp 

gdp_median <- stats[[16, 2]]

avg_gdp <- df %>% # average gdp over time 
...
for (i in 1:nrow(avg_gdp)){
  if (avg_gdp[i, 2] >= gdp_median){
    gdp_high <- append(gdp_high, avg_gdp[[i, 1]]) # la mediana è un quantile
                       }  else {
