...
for (i in 1:nrow(avg_gdp)){
  if (avg_gdp[i, 2] >= gdp_median){
    gdp_high <- append(gdp_high, avg_gdp[[i, 1]])
                       }  else {
...
df %>% 
  ...
  ggplot(aes(x = patent_count)) + geom_histogram(binwidth = 3, linewidth = 3, fill= "lightblue") + theme_bw() + theme_bw() + ggtitle("Distribution per high patenting states")

df %>% 
  filter(state_name %in% y_low) %>% 
  ggplot(aes(x = patent_count)) + geom_histogram(binwidth = 3, linewidth = 3, fill= "lightblue") + theme_bw() + theme_bw() + ggtitle("Distribution per low patenting states")
