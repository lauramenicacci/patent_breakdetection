...
ggsave(".\\clean_timeseries.png", plot = clean_ts, device = "png", width = 10, height = 10)
...
pat_plot <- pat %>%
  ...
  select(appln_id, CPC_Class, prio_year) %>% 
...
top10_techs <- pat_plot2 %>% 
  drop_na() %>% 
  count(Name) %>% 
  arrange(desc(n)) %>% 
  head(10) %>% 
  ggplot(aes(reorder(Name, n), n)) +
  geom_col(fill = "#280084", alpha = 0.7) + 
  coord_flip() +
  ylab("") + xlab("") +
  theme_bw() +
  theme(axis.text.y = element_text(hjust = 1, size= 7, color="Black")) +
  ggtitle("")

ggsave(".\\top10_techs.png", plot = top10_techs, device = "png", width = 7, height = 4)
...
techs <- c("Nuclear fission reactors", "Energy storage using batteries", "Fuel cells", "Wind turbines with rotation axis in wind direction", "Power conversion electric or electronic aspects")
...
techs <- c("Biofuels", "Energy storage using batteries","Fuel cells", "Photovoltaic [PV] energy", "Energy storage using capacitors")
...
pol <- read_csv("C:/Users/laura/OneDrive/Documenti/LAURA/HERTIE 22-23/THESIS/patent_breakdetection/policy_attribution/policy_dataset1.csv", show_col_types = FALSE) %>% select(state_name, policy_name, policy_type) %>% group_by(state_name) %>% count
...
scat <- p_c %>% 
  ...
  geom_point(size=2, color = "#280084") +
...
ggsave("Scatt.png", plot = scat, device = "png", width = 5, height = 5)

cor(p_c$tot_count_per_state, p_c$n)
